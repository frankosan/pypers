...
This is a modified version for use by nespipe, which bypasses JSON-RPC calls
All references to Context object are removed.
For kmer_v1 use the implemetantion, not the client.
...
sub new
{
    ...
    my $cfg = Bio::KBase::DeploymentConfig->new("genome_annotation");
    use Data::Dumper;
    print "Using config file ", $ENV{KB_DEPLOYMENT_CONFIG}, "\n";

    my $dir = $cfg->setting("kmer_v2_data_directory");
    $dir or warn"Configuration parameter for kmer_v2_data_directory not set";
    -d $dir or warn "Directory $dir for kmer_v2_data_directory does not exist";
...
sub create_genome
{
	...
	my $idc = Bio::KBase::IDServer::Client->new($idserver_url);
	if ($idc->can("get_identifier_prefix"))
	{
	    $id_prefix = $idc->get_identifier_prefix();
	}
...
sub call_selenoproteins
{
    ...
    {
	die "rast_call_special_proteins failed: ";
    }
...
sub call_features_CDS_glimmer3
{
	...
	$calls = Bio::KBase::GenomeAnnotation::Glimmer::call_genes_with_glimmer($sequences_file, $gparams);
    };
...
sub annotate_proteins_kmer_v1
{
    ...
    my $kmer_service = Bio::KBase::KmerAnnotationByFigfam::KmerAnnotationByFigfamImpl->new();
...
sub annotate_proteins_kmer_v2
{
    ...
    print "Runnning @cmd < $sequences_file > $output_file\n";
    my $ok = run(\@cmd,
		 "<", $sequences_file,
		 ">", $output_file);
...
sub call_features_ProtoCDS_kmer_v1
{
    ...
    # Us the implemetation, not the client...
    #my $kmer_service = Bio::KBase::KmerAnnotationByFigfam::Client->new($self->{kmer_service_url});
    my $kmer_service = Bio::KBase::KmerAnnotationByFigfam::KmerAnnotationByFigfamImpl->new();
    if (!defined($params->{dataset_name}))
    {
